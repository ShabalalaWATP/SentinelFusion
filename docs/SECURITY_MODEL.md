...
- `/analysis`, API-side enrichment routes, FIRMS fire context when a provider key is configured, and credential-backed conflict/protest context validate input with Zod and can be guarded with `ANALYSIS_API_TOKEN`.
...
- `/context/fire-anomalies` requires `ANALYSIS_API_TOKEN` before provider access when a live `FIRMS_MAP_KEY` is configured, accepts only validated area bounds, builds requests to the fixed NASA FIRMS host, keeps `FIRMS_MAP_KEY` server-side, splits antimeridian boxes into valid provider requests, parses CSV into typed/capped detections, uses timeout and bounded cache controls, and returns typed provider states without exposing provider request URLs or credentials.
