...
- Protected NASA FIRMS active-fire area context using server-side `FIRMS_MAP_KEY`, with `ANALYSIS_API_TOKEN` required before live provider access when a key is configured.
...
- [x] `pnpm lint`, `pnpm typecheck`, `pnpm test`, and `pnpm build` pass after the removal cleanup.

## Verification Record

Previous completed slices passed the required checks at the time they were implemented. The removal cleanup and later security/documentation pass were verified again with:
...
- ACLED and FIRMS still need configured API-side credentials for live provider-backed context. ACLED credentials and `FIRMS_MAP_KEY` also require `ANALYSIS_API_TOKEN`.
