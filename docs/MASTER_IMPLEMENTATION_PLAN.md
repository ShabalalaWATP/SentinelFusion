...
- Server-side NASA FIRMS active-fire context for analysed areas, with `FIRMS_MAP_KEY` held in the API, `ANALYSIS_API_TOKEN` required before live provider access when a key is configured, strict bounds validation, antimeridian-aware provider requests, capped detections, `ok`/`not_configured`/`error` states, and a toggleable fire-points map overlay.
